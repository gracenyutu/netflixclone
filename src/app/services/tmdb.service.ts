...
export class tmdbService implements OnInit  {
  ...
  getMovie(id: any): Observable<Movies> {
    return this.http.get<Movies>(`${this.url}${endpoint.movieID}${id}?&language=en-US`, { params })
  }
  ...
  getSeries(): Observable<Movies> {
    return this.http.get<Movies>(`${this.url}${endpoint.series}?&language=en-US`, { params })
  }
